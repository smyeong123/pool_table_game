...
//        this.pockets.add(new Pocket(dimX - POCKET_OFFSET, dimY - POCKET_OFFSET));
    ...
    public void setPockets(List<Pocket> pockets) {
        ...
        this.shape = new Rectangle(this.dim[0], this.dim[1], this.colour);
        this.shape.relocate(pockets.get(0).getCircle().getCenterX(), pockets.get(0).getCircle().getCenterY());
    }
    /**
    ...
    /**
     * Hard copy the list of balls, which includes the cueball
     * @param balls The list of balls to be set.
     */
    ...
    public void rollBack(List<Ball> cBalls) {
        for(int i = 0; i < this.balls.size(); i++) {
            this.balls.get(i).setXPos(cBalls.get(i).getXPos());
            this.balls.get(i).setYPos(cBalls.get(i).getYPos());
            this.balls.get(i).setXVel(cBalls.get(i).getXVel());
            this.balls.get(i).setYVel(cBalls.get(i).getYVel());
            this.balls.get(i).setDisabled(cBalls.get(i).getDisable());
            if(!this.balls.get(i).isDisabled()) {
                this.balls.get(i).makeVisible();
            }
            this.balls.get(i).setFallCounter(cBalls.get(i).getFallCounter());
        }
    }
...
//        Bounds tableBounds = this.shape.getBoundsInLocal();
        Bounds tableBounds = this.shape.getBoundsInParent();
